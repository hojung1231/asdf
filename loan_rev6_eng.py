    ...
    if checked:
        for idx, label in enumerate(year_labels):
            if label in checked:
                ...
                ax.bar(df["월"], df["남편 실수령액"], label="Husband", color="#5B9BD5")
                ax.bar(df["월"], df["아내 실수령액"], label="Wife", bottom=df["남편 실수령액"], color="#ED7D31")
                for i, total in enumerate(df["합계"]):
                    ax.text(i, total + 2, f"{int(total):,}만원", ha='center', va='bottom', fontsize=6, fontweight='bold')
                ax.set_ylabel("Monthly Net Salary (10,000 KRW)")
                ax.set_xlabel("Month")
                ax.set_title(f"{label} Couple Monthly Net Income (Stacked, Parental Leave Applied)")
                ax.legend()
                st.pyplot(fig)
...
# ---- 두번째 페이지 ----
    ...
    if house_mode == "매매":
        ...
        ax.plot(df["연도"], df["순자산(상승시)"], marker='o', label="Net Assets (House Price Up)")
        ax.plot(df["연도"], df["순자산(하락시)"], marker='o', label="Net Assets (House Price Down)")
        ax.set_xlabel("Year")
        ax.set_ylabel("Net Assets (10,000 KRW)")
...
# ---- 세번째 페이지 ----
    ...
    for year_label in checked:
        ...
        ax.set_ylabel("Monthly Expenses (10,000 KRW)")
        ax.set_xlabel("Month")
        ax.set_title(f"{year} Monthly Expenses (Stacked, Inflation Applied)")
        ax.legend()
        st.pyplot(fig)
...
# ---- 네번째 페이지 ----
    ...
    if view_mode == "연도별":
        ...
        ax.set_ylabel("Annual Amount (10,000 KRW)")
        ...
        ax.set_ylabel("Monthly Amount (10,000 KRW)")
        ax.set_title(f"{sel_start_year}~{sel_start_year+sel_period-1} Monthly Income / Expense / Net Savings")
        if label_shown:
            ax.legend()
        st.pyplot(fig)
        ...
        ax2.set_ylabel("Cumulative Cash Flow (10,000 KRW)")
